...
import { createModulSchema, updateModulSchema, getAllModulParamsSchema } from "./modul-schema.js";
...
class ModulRoutes extends BaseRoutes {
  routes() {
    this.router.get(
      ...
      authTokenMiddleware.authenticate, tryCatch(modulController.getAll)
    );

    this.router.get(
      "/:id",
      authTokenMiddleware.authenticate, tryCatch(modulController.getById)
    );
    ...
    this.router.delete(
      "/:id",
      authTokenMiddleware.authenticate, tryCatch(modulController.remove)
    );
  }
}
